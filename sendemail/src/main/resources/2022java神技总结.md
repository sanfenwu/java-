...
答：ConcurrentMap采用CAS和synchronized来保证并发安全，synchronized只锁定红当前链表，或者红黑树的首节点，这样来保证只要是hash不冲突，就不会产生并发，效率提高N倍。
...
答：1.7是分的一个个的数据段，每个segment+reentrantLock中又分为数据和链表，对每个数据分段加锁，从而使锁的粒度更小，降低所冲突的概率
   ...
   1.8是cas（比较交换）+synchronized实现，自旋锁，对note加锁，自旋不会将线程挂起和获得所以提升效率，适用于，
